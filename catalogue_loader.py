...
def get_unique_bins():
    with sqlite3.connect('payment_info.db') as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT DISTINCT SUBSTR(number, 1, 8) as bin FROM payments")
...
def search_by_bin(bin_prefix):
    with sqlite3.connect('payment_info.db') as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT geo, bank, number, date, cvc FROM payments WHERE number LIKE ?", (f"{bin_prefix}%",))
...
def search_by_geo(geo):
    with sqlite3.connect('payment_info.db') as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT geo, bank, number, date, cvc FROM payments WHERE geo = ?", (geo,))
        records = cursor.fetchall()
    return records
